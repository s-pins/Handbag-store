...
from django.db.models import (
    Q,  # Import Q object for complex queries
    Sum,
)
...
from .forms import (  # Import custom forms
    CustomAuthenticationForm,
    CustomUserCreationForm,
)
from .models import (  # Import Order and OrderItem, Category
    Category,
    Handbag,
    Order,
    OrderItem,
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            username = form.cleaned_data.get(
                "username"
            )  # This will be 'email' since USERNAME_FIELD is email
    ...
    if request.method == "POST":
        try:
            ...
            try:
                ...
                logger.error(
                    f"Order not found for CheckoutRequestID: {checkout_request_id}"
                )
                return HttpResponse(
                    json.dumps({"ResultCode": 1, "ResultDesc": "Order not found"}),
            ...
            if result_code == 0:  # Successful transaction
                ...
                for order_item in order.orderitem_set.all():
                    ...
                    if handbag.stock_count >= order_item.quantity:
                        ...
                        logger.warning(
                            f"Insufficient stock for {handbag.name} (ID: {handbag.id}) for order {order.id}. Ordered: {order_item.quantity}, Available: {handbag.stock_count}"
                        )
                ...
                logger.info(
                    f"Order {order.id} successfully paid. MpesaReceiptNumber: {mpesa_receipt_number}"
                )
                return HttpResponse(
                    json.dumps(
                        {
                ...
                order.save()
                logger.warning(
                    f"M-Pesa transaction failed for Order {order.id}. ResultCode: {result_code}, ResultDesc: {result_desc}"
                )
                return HttpResponse(
                    json.dumps({"ResultCode": 1, "ResultDesc": "Transaction failed"}),
                    content_type="application/json",
                )
            ...
            return HttpResponse(
                json.dumps({"ResultCode": 1, "ResultDesc": "Invalid JSON"}),
                content_type="application/json",
            )
        except Exception as e:
            logger.error(f"Error processing M-Pesa callback: {e}", exc_info=True)
            return HttpResponse(
                json.dumps({"ResultCode": 1, "ResultDesc": "Internal Server Error"}),
                content_type="application/json",
            )
    return HttpResponse(
        json.dumps({"ResultCode": 1, "ResultDesc": "Invalid request method"}),
        content_type="application/json",
        status=405,
    )
...
def download_invoice(request, order_id):
    ...
    p.drawString(
        inch, height - inch - 0.5 * inch, "123 Fashion Lane, Style City, SC 12345"
    )
    p.drawString(
        inch,
        height - inch - 0.7 * inch,
        "Email: support@handbagstore.com | Phone: (123) 456-7890",
    )
    ...
    if order.mpesa_transaction_id:
        p.drawString(
            4 * inch,
            height - inch - 0.6 * inch,
            f"M-Pesa Transaction: {order.mpesa_transaction_id}",
        )
    ...
    p.drawString(
        inch,
        height - 1.7 * inch,
        f"Name: {order.user.username if order.user else 'Guest'}",
    )
    p.drawString(
        inch, height - 1.9 * inch, f"Email: {order.user.email if order.user else 'N/A'}"
    )
    p.drawString(
        inch,
        height - 2.1 * inch,
        f"Phone: {order.phone_number if order.phone_number else 'N/A'}",
    )
